...
export default function ClientQuotationPage() {
  ...
  const { client, loading, error, mutate } = useClientDetail(clientId || null, 'quotation');
    ...
    return (
      <main className="min-h-screen bg-slate-50 p-4 sm:p-6">
        <div className="mx-auto max-w-5xl space-y-4 rounded-2xl border border-amber-200 bg-amber-50 p-6 text-amber-900">
          <h1 className="text-lg font-bold">عرض السعر</h1>
          <p>لم يتم تحديد عميل لعرض سعره.</p>
    ...
    return (
      <main className="min-h-screen bg-slate-50 p-4 sm:p-6">
        <div className="mx-auto max-w-5xl space-y-4 rounded-2xl border border-rose-200 bg-rose-50 p-6 text-rose-900">
          <h1 className="text-lg font-bold">تعذر تحميل عرض السعر</h1>
          <p>{error instanceof Error ? error.message : 'تعذر الوصول إلى بيانات العميل أو عرض السعر.'}</p>
          <button type="button" onClick={() => router.push('/sales')} className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white">
            العودة إلى المبيعات
          </button>
        </div>
