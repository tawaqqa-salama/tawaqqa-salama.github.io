...
export function IncomeComparisonChart({
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
      <h3 className="font-bold text-gray-800 mb-4">ملخص قائمة الدخل</h3>
      <div className="space-y-4">
        <BarRow label="الإيرادات" value={revenue} max={max} colorClass="bg-emerald-500" />
        <BarRow label="المصروفات" value={expenses} max={max} colorClass="bg-rose-500" />
  ...
  return (
    <div>
      ...
      <div className="h-4 bg-gray-100 rounded-full overflow-hidden">
        <div className={`h-full rounded-full ${colorClass}`} style={{ width: `${Math.max((value / max) * 100, 4)}%` }} />
      </div>
    </div>
  );
}
