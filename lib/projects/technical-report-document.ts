...
export function generateTechnicalReportDocument(params: {
  client: ClientRecord;
  report: TechnicalReport;
  engineeringData?: ProjectEngineeringData | null;
  locale?: ReportLocale;
  /** Ephemeral print-only media measurements; never persisted to the report. */
  ...
  const selectedCodeEvidence = pdfContent.evidence_groups.some((group) => group.group === 'code_evidence');
  const applicableCodes = selectedCodeEvidence ? ['مراجع ومقتطفات فنية اختار المهندس إدراجها ضمن التقرير.'] : [];
  const hasMechanical = Boolean(notes(report.ventilation_items).length);

  const evidenceSectionId: Record<TechnicalReportPdfEvidenceGroup, EngineeringStudySection['id']> = {
  ...
  const evidenceSections = pdfContent.evidence_groups.map((group, index) => {
    const fallbackParagraphs = group.items
      .filter((item) => !item.image_src)
      .flatMap((item) => [
        ...(item.kind === 'code_excerpt' ? codeEvidenceReferenceLines(item.code_reference) : []),
        ...(item.engineering_note ? [`ملاحظة المهندس: ${item.engineering_note}`] : []),
      ]);
      ...
      .map((item) => ({
        ...
        caption_ar:
          item.kind === 'code_excerpt'
            ? `مقتطف مرجعي مرفق بالدراسة — ${item.caption}`
            : item.caption,
        ...
        description_ar:
          item.kind === 'code_excerpt'
            ? codeEvidenceReferenceLines(item.code_reference).join(' — ') || item.engineering_note
            : item.engineering_note,
        description_en: item.engineering_note,
      }));
    return section(
      evidenceSectionId[group.group],
      16 + index / 10,
      group.title_ar,
      [...group.paragraphs, ...fallbackParagraphs],
      [],
      images
    );
  });
  ...
  const sections: EngineeringStudySection[] = [
    ...
    section('site_information', 5, 'موقع المشروع', [`المدينة: ${value(p.city)}. الحي: ${value(p.district)}. الشارع: ${value(p.street)}. العنوان: ${value(p.national_address)}. الإحداثيات: ${report.gps_lat && report.gps_lng ? `${report.gps_lat}، ${report.gps_lng}` : '—'}. ${report.location_description || ''}`]),
    ...
    ...(pdfContent.recommendations.length ? [section('engineering_recommendations', 18, 'التوصيات', pdfContent.recommendations.map((item) => item.text))] : []),
