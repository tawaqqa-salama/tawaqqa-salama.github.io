...
export function generateTechnicalReportDocument(params: { client: ClientRecord; report: TechnicalReport; engineeringData?: ProjectEngineeringData | null; locale?: ReportLocale }): EngineeringStudyDocument {
  ...
  const sections: EngineeringStudySection[] = [
    ...
    section('hazard_classification', 8, 'الهيكل الإنشائي ومتطلبات مقاومة الحريق', [], [table('الخصائص الإنشائية المتاحة', ['البند', 'القيمة'], [['نوع البناء', value(plan.construction_type)], ['التصنيف الإنشائي', value(plan.occupancy_classification)], ['مبنى مرتفع', value(plan.high_rise_building)], ['أتريوم', value(plan.atrium_exists)], ['عدد الأقبية', n(plan.basement_floors_count)], ['العمق تحت الأرض', n(plan.underground_depth_m, 'م')], ['مبنى بلا نوافذ', value(plan.windowless_building)]])]),
    section('means_of_egress', 9, 'دراسة الطاقة الاستيعابية', [`إجمالي عدد الشاغلين المسجل: ${n(aggregates.total_occupants)}. لا تُحوّل القيم غير المدخلة إلى صفر.`], [table('حصر الشاغلين', ['الدور', 'المساحة / الاستخدام', 'المساحة', 'عدد الشاغلين'], occupantRows)]),
    section('fire_truck_access', 10, 'مخارج ومسارات الهروب', [], [table('المخارج ومسارات الهروب', ['الدور', 'الشاغلون', 'المخارج', 'السلالم', 'أقصى مسافة سفر'], egressRows)],),
    ...
    section('portable_extinguishers', 12.2, 'الطفايات اليدوية', [], [table('حصر الطفايات اليدوية', ['الدور', 'المساحة', 'العدد', 'النوع', 'السعة'], extinguisherRows)]),
    section('fire_alarm_study', 13, 'نظام الإنذار المبكر من الحريق', [], [table('ملخص الإنذار والكواشف', ['الدور', 'المساحة', 'لوحات الإنذار', 'كواشف الدخان', 'كواشف الحرارة', 'أجهزة التنبيه'], alarmRows)]),
    section('emergency_lighting', 13.1, 'إنارة الطوارئ واللوحات الإرشادية', [], [table('حصر إنارة الطوارئ واللوحات الإرشادية', ['الدور', 'المساحة', 'إنارة الطوارئ', 'اللوحات الإرشادية'], emergencyRows)]),
    section('electrical_safety', 14, 'متطلبات السلامة الكهربائية', [], [table('السلامة الكهربائية', ['البند', 'القيمة'], [['التأريض', value(plan.electrical_grounding, 'غير مدخل')], ['الحماية من الصواعق', value(plan.lightning_protection, 'غير مدخل')], ['المولد الاحتياطي', value(plan.backup_generator, 'غير مدخل')], ['ملاحظات المهندس', report.overview_text || '—']])]),
