...
function normalizeDisplay(value: string | null | undefined): string {
  ...
  return raw.replace(/\bK-Factor\s+K(?=\d)/gi, 'K-Factor = ').replace(/^K(\d+(?:\.\d+)?)$/, 'K = $1');
}
...
function renderSystem(system: UnderConstructionTechnicalReportSystem): string {
  const references = system.canonical_references.map((item) => `<tr><th>${tx(item.label)}</th><td>${display(item.value)}</td><td class="source-cell">${tx(sourceLine(item))}</td></tr>`).join('');
  return `<article class="system-block keep">
    <div class="system-heading"><h3>${tx(system.system_label)}</h3><span>${system.applicable === true ? 'منطبق بقرار صريح' : 'قرار الانطباق غير مكتمل'}</span></div>
    <table class="system-table"><tbody>
      <tr><th>المتطلب حسب الكود / التصميم</th><td colspan="2">${display(system.code_requirement)}</td></tr>
      <tr><th>الحل التصميمي المختار</th><td colspan="2">${display(system.selected_solution)}</td></tr>
      <tr><th>مرجع الكود</th><td colspan="2">${display(system.code_reference)}</td></tr>
      <tr><th>مرجع المخطط / التصميم</th><td colspan="2">${display(system.drawing_reference)}</td></tr>
      <tr><th>مرجع الحساب</th><td colspan="2">${display(system.calculation_reference)}</td></tr>
      <tr><th>ملاحظات التنفيذ</th><td colspan="2">${display(system.implementation_note)}</td></tr>
    </tbody></table>
    <div class="subcaption">القيم الهندسية المرجعية — للعرض فقط من المصادر الكانونية</div>
    <table class="reference-table"><thead><tr><th>البند</th><th>القيمة</th><th>المصدر</th></tr></thead><tbody>${references || '<tr><td colspan="3" class="empty-cell">لا توجد قيمة مصدرية موثقة لهذا النظام.</td></tr>'}</tbody></table>
  </article>`;
}
...
function renderToc(pageMap?: Record<string, number>): string {
  const entries = [
    ['intro', 'مقدمة ونطاق الدراسة'], ['project', 'بيانات المشروع والمالك'], ['building', 'بيانات المبنى والتصنيف'], ['basis', 'أساس التصميم والمراجع'], ['requirements', 'متطلبات السلامة والوقاية من الحريق'], ['engineering', 'الأنظمة الهندسية المطلوبة'], ['data', 'البيانات والحسابات الهندسية المرجعية'], ['recommendations', 'المتطلبات والتوصيات الفنية'], ['summary', 'الملخص والخلاصة وحدود الدراسة'],
  ];
...
function renderSystems(sections: UnderConstructionTechnicalReportSection[]): string {
  if (!sections.length) return '<div class="empty-note">لم تُسجل قرارات دراسة صريحة للأنظمة بعد؛ لا يعني ذلك عدم الحاجة إلى أي نظام.</div>';
  return sections.map((section) => `<div class="system-section"><h3 class="subheading">${tx(section.label)}</h3>${section.systems.map(renderSystem).join('')}</div>`).join('');
}
...
export function buildUnderConstructionFinalTechnicalReportHtml(params: {
  ...
  const body = [
    ...
    renderSection(5, 'متطلبات السلامة والوقاية من الحريق', `<p class="lead">تظهر البنود ذات القرار الصريح فقط. غياب البند لا يعني الحكم بعدم الحاجة إليه.</p>${renderSystems(model.report_sections.slice(0, 2))}`, 'requirements', pageMap),
    renderSection(6, 'الأنظمة الهندسية المطلوبة', renderSystems(model.report_sections), 'engineering', pageMap),
    renderSection(7, 'البيانات والحسابات الهندسية المرجعية', `<p class="lead">القيم التالية للعرض المرجعي من مصادر التصميم الأصلية، ولا يعيد التقرير حسابها أو تخزين نسخة موازية منها.</p>${renderSystems(model.report_sections)}`, 'data', pageMap),
    renderSection(8, 'المتطلبات والتوصيات الفنية', renderRecommendations(model), 'recommendations', pageMap),
    renderSection(9, 'الملخص والخلاصة وحدود الدراسة', `<div class="summary-box"><p>يعرض هذا التقرير الدراسة الفنية ومتطلبات أنظمة السلامة والوقاية من الحريق وفق البيانات والمستندات المتاحة للمشروع، وتبقى مسؤولية استكمال المتطلبات والتنفيذ والاعتمادات وفق الإجراءات النظامية والجهات المختصة.</p><ul class="limitation-list">${model.limitations.map((item) => `<li>${tx(item)}</li>`).join('')}</ul></div>`, 'summary', pageMap),
  ].join('');
