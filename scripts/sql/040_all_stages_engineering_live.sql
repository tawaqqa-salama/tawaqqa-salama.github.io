-- ============================================================================
-- ALL STAGES FIX — run THIS file ONLY once in Supabase SQL Editor.
-- You do NOT need to run 035 / 036 / 037 / 038 / 039 separately.
-- Saves for every project stage go to project_engineering_live (small JSON),
-- never rewriting the fat clients.project_engineering_data column.
-- Safe to re-run.
-- ============================================================================

-- 1) Universal live payload for ALL workflow stages
CREATE TABLE IF NOT EXISTS public.project_engineering_live (
...
CREATE INDEX IF NOT EXISTS idx_project_engineering_live_updated
  ON public.project_engineering_live (updated_at DESC);
...
BEGIN
  ...
  -- Tiny clients columns only — NEVER touch project_engineering_data
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'project_engineering_live'
      AND policyname = 'project_engineering_live_all'
  ) THEN
    CREATE POLICY project_engineering_live_all ON public.project_engineering_live
      FOR ALL USING (true) WITH CHECK (true);
  END IF;
END $$;

GRANT SELECT, INSERT, UPDATE, DELETE ON public.project_engineering_live
  TO anon, authenticated;

-- Optional: slim bloated legacy column in background (does not block live saves)
...
-- ============================================================================
-- 2) Stage-5 relational helpers (visits / supervision / PDF index) — included so
--    you never need to run 038 separately.
-- ============================================================================
