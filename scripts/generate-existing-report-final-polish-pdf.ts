...
const outDir = join(process.cwd(), 'artifacts/existing-report-final-polish');
mkdirSync(outDir, { recursive: true });

const client: ClientRecord = {
  id: 'final-polish-artifact',
  client_code: 'LD-POL-ART',
  name: 'Final Polish Artifact',
  ...
  primary_engineering_project_identity: {
    clientId: 'final-polish-artifact',
    projectId: 'p1',
    projectCode: 'PRJ-POL-ART',
...
const data = parseProjectEngineeringData({
  ...
  technical_report: {
    ...
    outgoing_number: 'TR-POL-ART',
    report_date: '2026-08-29',
    location_description: 'artifact final polish',
  ...
  existing_assessment: {
    version: 1,
    systems: {
      fire_truck_access: { applicable: true, existing_presence: 'PRESENT', compliance_status: 'NEEDS_COMPLETION', action_text: 'إجراء وصول.' },
...
async function main() {
  ...
  if (html.includes(`>${LONG_MAPS_URL}<`)) {
    throw new Error('Long Google Maps URL leaked into visible table text.');
  }
  ...
  const result = {
    ...
    longMapsUrlPrintedInHtml: html.includes(`>${LONG_MAPS_URL}<`),
    ...
    arabicExtraction: {
      ...
      sampleExtract: extraction.text.slice(0, 500),
      foldedSample: extraction.folded.slice(0, 200),
      diagnosis: extraction.engine === 'pdfjs'
        ? 'Chromium embeds Noto Naskh as vector text; pdf.js may still split Arabic glyphs during extraction even when copy/search in a viewer is correct.'
        : 'pdftotext extraction used; closer to desktop copy/search behavior.',
    },
